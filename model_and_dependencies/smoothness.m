...
if ((Nx>1) && (Ny==1)) || ((Nx==1) && (Ny>1)) % 1D
    ...
    % Upwind divided difference
    df_up = (q>=0).*[df(Nf);df(1:Nf-1)] + (q<0).*[df(2:Nf);df(1)];
    ...
    DX_up = (reshape(q(1:Grid.Nfx),Ny,Nx+1)>=0).*[DX(:,Nx+1),DX(:,1:Nx)] +...
            (reshape(q(1:Grid.Nfx),Ny,Nx+1)< 0).*[DX(:,2:Nx+1),DX(:,1)];
